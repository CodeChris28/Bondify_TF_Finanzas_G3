...
class FinancialService {
  Map<String, dynamic> calculateBondResults({
    ...
    final double couponFrequencyInDays = _getCouponFrequencyInDays(
    ...
    final double capitalizationInDays = _getCapitalizationInDays(
      bond.capitalization,
    );
    ...
    if (bond.interestRateType.toLowerCase() == 'efectiva') {
      ...
      if (capitalizationInDays == 0)
        throw Exception('Capitalización no definida para tasa nominal.');
    ...
    final cokPeriodo =
        pow(
          (1 + (bond.annualDiscountRate / 100)),
          (couponFrequencyInDays / bond.daysPerYear),
        ) -
        1;
    ...
    if (formProvider.selectedEstructuracionPayer != 'Bonista') {
      emisorCostsPercent += bond.structuringCostPercent / 100;
    }
    if (formProvider.selectedColocacionPayer != 'Bonista') {
      emisorCostsPercent += bond.placementCostPercent / 100;
    }
    // CORRECCIÓN: Añadir Flotación y CAVALI a los costos del emisor si es 'Ambos'
    if (formProvider.selectedFlotacionPayer != 'Bonista') {
      // O una lógica más explícita
      emisorCostsPercent += bond.flotationCostPercent / 100;
    }
    if (formProvider.selectedCavaliPayer != 'Bonista') {
      // O una lógica más explícita
      emisorCostsPercent += bond.cavaliCostPercent / 100;
    }

    // Lógica para costos del bonista
    if (formProvider.selectedFlotacionPayer != 'Emisor') {
      bonistaCostsPercent += bond.flotationCostPercent / 100;
    }
    if (formProvider.selectedCavaliPayer != 'Emisor') {
    ...
    final double cuota =
        currentBalance *
        ((tep * pow(1 + tep, totalPeriods)) / (pow(1 + tep, totalPeriods) - 1));

    for (int i = 1; i <= totalPeriods; i++) {
      final interest = currentBalance * tep;
      final amortization = cuota - interest;
      final shield = interest * (bond.incomeTax / 100);
      final double finalAmortization = (i == totalPeriods)
          ? currentBalance
          : amortization;
      final double finalCoupon = interest + finalAmortization;
      final double prima = (i == totalPeriods)
          ? -bond.nominalValue * (bond.initialCostPercent / 100)
          : 0;

      final double flujoEmisor = -finalCoupon + prima;
      final double flujoEmisorConEscudo = flujoEmisor + shield;
      final double flujoBonista = -flujoEmisor;
      ...
      cashFlowTable.add(
        CashFlowRow(
          ...
          date: Timestamp.fromDate(
            bond.issueDate.toDate().add(
              Duration(days: (couponFrequencyInDays * i).round()),
            ),
          ),
